...
public class SmsPopupActivity extends FragmentActivity implements
	...
	private void setupPreferences() {
		...
		// prefs
		if (mPrefs.getBoolean(getString(R.string.pref_autorotate_key),
				Defaults.PREFS_AUTOROTATE)) {
		...
		// Fetch privacy mode
		final boolean privacyMessage = mPrefs.getBoolean(
				getString(R.string.pref_privacy_key), Defaults.PREFS_PRIVACY);
		final boolean privacySender = mPrefs.getBoolean(
				getString(R.string.pref_privacy_sender_key),
				Defaults.PREFS_PRIVACY_SENDER);
		privacyAlways = mPrefs.getBoolean(
				getString(R.string.pref_privacy_always_key),
		...
		showUnlockButton = mPrefs.getBoolean(
				getString(R.string.pref_useUnlockButton_key),
				Defaults.PREFS_USE_UNLOCK_BUTTON);

		// Fetch quick reply signature
		signatureText = mPrefs.getString(
				getString(R.string.pref_notif_signature_key), "");
	...
	private class LoadUnreadMessagesAsyncTask extends
			AsyncTask<SmsMmsMessage, Void, ArrayList<SmsMmsMessage>> {
		...
		protected ArrayList<SmsMmsMessage> doInBackground(SmsMmsMessage... args) {
			...
			ArrayList<SmsMmsMessage> messages = SmsPopupUtils
					.getUnreadMessages(SmsPopupActivity.this);
			...
			if (messages == null) {
				...
				if (messageId == 0) {
					// Add the new message as it wouldn't have been found in the
					// system database
					messages.add(newMessage);
				} else {
					// Otherwise we need to check if getUnreadMessages() found
					// the new message,
					...
					for (int i = 0; i < messages.size(); i++) {
						if (messages.get(i).getMessageId() == messageId) {
							...
							android.util.Log.i("new messages", messages.get(i)
									.getMessageId() + "");
					...
					// See {@link SmsPopupUtils.updateSmscTimestampDrift} for
					// why this is needed.
					final long smscTimeDrift = mPrefs.getLong(
							ManagePreferences.SMSC_TIME_DRIFT, 0);
					if (smscTimeDrift > 0) {
						for (int i = 0; i < messages.size(); i++) {
	...
	/*
	 * *****************************************************************************
	 * Methods that will be called several times throughout the life of the
	 * activity
	 * *****************************************************************
	 * ************
	 */
	...
	private void resizeLayout() {
		final int width = (int) getResources().getDimension(
				R.dimen.smspopup_pager_width);
		final int height = (int) getResources().getDimension(
				R.dimen.smspopup_pager_height);
		final int screenWidth = getResources().getDisplayMetrics().widthPixels;
		final int bottom_padding = (int) getResources().getDimension(
				R.dimen.smspopup_bottom_margin);
		...
		RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) smsPopupPager
				.getLayoutParams();
	...
	/**
	 * Customized activity finish. Ensures the notification is in sync and
	 * cancels any scheduled reminders (as the user has interrupted the app).
	 */
	private void myFinish() {
		...
		if (inbox) {
			...
			// Start a service that will update the notification in the status
			// bar
			...
			if (replying) {
				...
				// We need to know if the user is replying - if so, the entire
				// thread id should
				// be ignored when working out the message tally in the
				// notification bar.
				// We can't rely on the system database as it may take a little
				// while for the
				// reply intent to fire and load up the messaging up (after
				// which the messages
	...
	public void onWindowFocusChanged(boolean hasFocus) {
		...
		if (hasFocus) {
			...
			// was at some
			// point visible. I tried using onResume() or other methods to
			// prevent doing some
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		menu.add(Menu.NONE, CONTEXT_VIEWCONTACT_ID, Menu.NONE,
				getString(R.string.view_contact));
		menu.add(Menu.NONE, CONTEXT_CLOSE_ID, Menu.NONE,
				getString(R.string.button_close));
		menu.add(Menu.NONE, CONTEXT_DELETE_ID, Menu.NONE,
				getString(R.string.button_delete));
		menu.add(Menu.NONE, CONTEXT_REPLY_ID, Menu.NONE,
				getString(R.string.button_reply));
		menu.add(Menu.NONE, CONTEXT_QUICKREPLY_ID, Menu.NONE,
				getString(R.string.button_quickreply));
		menu.add(Menu.NONE, CONTEXT_TTS_ID, Menu.NONE,
				getString(R.string.button_tts));
		menu.add(Menu.NONE, CONTEXT_INBOX_ID, Menu.NONE,
				getString(R.string.button_inbox));
	}
...
//	}
	...
	private void closeMessage() {
		Intent intent = new Intent(getApplicationContext(),
				SmsPopupUtilsService.class);
		/*
		 * Switched back to mark messageId as read for >v1.0.6 (marking thread
		 * as read is slow for really large threads)
		 */
	 ...
	 */
	private void replyToMessage(final SmsMmsMessage message,
			final boolean replyToThread) {
	...
	/**
	 * View the private message (this basically just unlocks the keyg uard and
	 * then updates the privacy of the messages).
	 */
	...
	private void gotoInbox() {
		...
		ManageKeyguard.exitKeyguardSecurely(new LaunchOnKeyguardExit() {
			@Override
			public void LaunchOnKeyguardExitSuccess() {
				Intent i = SmsPopupUtils
						.getSmsInboxIntent(SmsPopupActivity.this);
	...
	public void onButtonClicked(int buttonType) {
		...
		case ButtonListPreference.BUTTON_REPLY: // Reply
			...
			Toast.makeText(getApplicationContext(),
					"ButtonListPreference.BUTTON_REPLY", Toast.LENGTH_SHORT)
					.show();
			// replyToMessage(true);
			break;
